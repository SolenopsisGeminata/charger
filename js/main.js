...
if($('.slider-picky').length>0){
	$('.slider-picky').slick({
		...
		arrows: false,
		//asNavFor: '.class',
		//appendArrows:'.main-slider__control-panel',
		//appendDots: '.class',
		//prevArrow:'.main-slider__left-arrow',
		//nextArrow:'.main-slider__right-arrow',
		//centerMode: false,
		//centerPadding: '50px',
		//cssEase: 'ease',
		dots: false,
...
if($('.slider-achievements').length>0){
	$('.slider-achievements').slick({
		...
		//rows: 1,
		slide: '.slider-achievements__item',
		//slidesPerRow: 1,
		//slidesToShow: 1,
		//slidesToScroll: 1,
		speed: 1000,
...
$('.slider-achievements__item').click(function(e) {
	$('.slider-achievements').slick('slickGoTo', e.currentTarget.dataset.slickIndex);
});
...
var mainSlider = new Swiper('.main-slider', {
	...
	on: {
		sliderMove: function () {
			if (!$('.main-slider__loading-circle').hasClass('rotating')) {
				$('.main-slider__loading-circle').addClass('rotating');
			}
		},
		...
		slideChangeTransitionEnd: function () {
			$('.main-slider__loading-circle').removeClass('rotating');
		},
		touchStart: function () {
			$('.main-slider__loading-circle').addClass('rotating');
		},
		touchEnd: function () {
			setTimeout( function() { $('.main-slider__loading-circle').removeClass('rotating') }, 1500);
		},
	},
});
*/
...
if($('.reviews-object__items').length>0){
	...
	function reviews_object(){
		slider.slick({
			...
			//asNavFor:'',
			//appendDots:
			//appendArrows:$('.main-slider-arrows .container'),
...
$('.form-field__label').click(function(e) {
	e.preventDefault();
});
/*
function setMainPaddingTop() {
	let main = $('.wrapper').children('.main').eq(0);
	let mainHeader = $('.wrapper').children('header').eq(0);
	console.log(mainHeader);
	let mainHeaderHeight = mainHeader.outerHeight();
	console.log(mainHeaderHeight)
	main.css('padding-top', mainHeaderHeight);
}
//$(document).ready(setFirstSectionPaddingTop);
//setFirstSectionPaddingTop();

$('.user-button__link').click(function() {
	let userMenu = $(this).siblings('.user-menu').eq(0);
	console.log(userMenu);
	userMenu.toggleClass('active');
});

$(document).click(function(e) {
	if ($(e.target).eq(0).closest('.user-button').length==0) {
		$('.user-button .user-menu').removeClass('active');
	}
});

$(window).resize( function() {
	//setTimeout(setMainPaddingTop, 500);
});

$('.lots-slider-arrow').mouseleave(	function() {
	$(this).addClass('finish-animation');
	setTimeout(function() {
		$('.lots-slider-arrow').removeClass('finish-animation');
	}, 20000);
});*/
});